...
<header>
  <h1><span id="t-title">手書き数字認識 AI</span> <button id="btn-lang" onclick="toggleLang()">EN</button></h1>
  <p class="subtitle" id="t-subtitle">指またはマウスで数字を描くと、ニューラルネットワークがリアルタイムで認識します</p>
  <p class="subtitle2" id="t-subtitle2">同時に、ネットワーク内部の信号の流れがリアルタイムに可視化されます</p>
  <p class="model-info" id="t-model-label">学習済みモデル &#9432;
    <span class="tooltip" id="model-tooltip"><span id="t-tooltip-body">MNIST（手書き数字の画像7万枚のデータセット）のうち<br>6万枚で学習し、残り1万枚でテストした正解率: {test_acc:.1%}<br>モデル構成: Conv(5x5,8ch) → Pool → Conv(3x3,16ch) → Pool → FC(64) → FC(10)<br><br>小さいながらも27,690パラメータ（GPT-4の6,500万分の1）を持つ<br>本物のニューラルネットワークです。<br>それでも98%の精度が出せるのでCNNの効率の良さがわかります。</span><br><button class="copy-btn" onclick="copyModelInfo(event)" id="t-copy-btn">コピー</button></span>
  </p>
</header>
...
<main>
  ...
  <section id="upper">
    ...
    <div id="canvas-wrap">
      ...
      <div id="cnn-input-preview">
        ...
        <span id="t-cnn-input">CNNの入力 (28×28)</span>
      </div>
    </div>
  </section>
  ...
  <section id="nn-diagram">
    <h2 id="t-nn-title">Network Diagram</h2>
...
def _js_ui(test_acc: float) -> str:
    acc_str = f"{test_acc:.1%}"
    return """
// ===================== i18n =====================
const LANG = {
  ja: {
    title: '手書き数字認識 AI',
    subtitle: '指またはマウスで数字を描くと、ニューラルネットワークがリアルタイムで認識します',
    subtitle2: '同時に、ネットワーク内部の信号の流れがリアルタイムに可視化されます',
    modelLabel: '学習済みモデル \\u2139',
    tooltipBody: 'MNIST（手書き数字の画像7万枚のデータセット）のうち<br>6万枚で学習し、残り1万枚でテストした正解率: """ + acc_str + """<br>モデル構成: Conv(5x5,8ch) → Pool → Conv(3x3,16ch) → Pool → FC(64) → FC(10)<br><br>小さいながらも27,690パラメータ（GPT-4の6,500万分の1）を持つ<br>本物のニューラルネットワークです。<br>それでも98%の精度が出せるのでCNNの効率の良さがわかります。',
    copyBtn: 'コピー',
    copiedBtn: 'コピーしました',
  ...
  en: {
    ...
    subtitle: 'Draw a digit with your finger or mouse — the neural network recognizes it in real time',
    subtitle2: 'Watch the signal flow through the network layers in real time',
    modelLabel: 'Trained Model \\u2139',
    tooltipBody: 'Trained on MNIST (70,000 handwritten digit images),<br>60,000 for training, 10,000 for testing — accuracy: """ + acc_str + """<br>Architecture: Conv(5x5,8ch) → Pool → Conv(3x3,16ch) → Pool → FC(64) → FC(10)<br><br>A real neural network with 27,690 parameters<br>(1/65-millionth of GPT-4), yet achieves 98% accuracy —<br>showing the efficiency of CNNs.',
    copyBtn: 'Copy',
    copiedBtn: 'Copied',
...
if (params.get('lang') === 'ja') currentLang = 'ja';
...
function setLang(lang) {
  ...
  document.getElementById('t-subtitle2').textContent = t.subtitle2;
  document.getElementById('t-model-label').firstChild.textContent = t.modelLabel + ' ';
  document.getElementById('t-tooltip-body').innerHTML = t.tooltipBody;
